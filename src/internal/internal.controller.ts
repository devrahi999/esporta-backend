...
export class InternalController {
  ...
  /**
   * Drains the media cleanup queue (R2 / Stream / Supabase Storage objects
   * orphaned by post purges and profile deletions). Scheduled like
   * push-dispatch: one authenticated request per interval from the external
   * scheduler; unclaimed rows wait for the next drain, so an interval of five
   * to fifteen minutes keeps the queue near-empty without dedicated compute.
   */
  @Post('media-cleanup')
  cleanup() {
    return this.mediaCleanup.drain();
  }

  /** GET — the shape the external scheduler sends for pull-style crons. */
  @Get('media-cleanup')
  cleanupCron() {
    return this.mediaCleanup.drain();
  }
