...
export class InviteMemberDto {
  ...
  // member = roster place, admin = administrative access (owner-only). `owner`
  // is never inviteable — ownership moves through transfer-owner.
  @IsOptional() @IsIn(['member', 'admin']) role?: string;
  @IsOptional() @IsString() game_role_slug?: string;
}

export class SetMemberRoleDto {
  // Same whitelist as the invite; the service refuses `owner` with a message
  // that points at transfer-owner instead of a bare validation error.
  @IsIn(['member', 'admin']) role!: string;
}
